...
def process_document_image(image_bytes, filename=None):
    ...
    try:
        ...
        image.save(tmp_path, format=img_format, optimize=True, quality=85)
        
        # Resize if too large
        while os.path.getsize(tmp_path)/1024 > 4000:
            w, h = image.size
            image = image.resize((int(w*0.8), int(h*0.8)), Image.LANCZOS)
            image.save(tmp_path, format=img_format, optimize=True, quality=85)

        # Extract raw OCR text with Gemini
        ...
        final = llm.generate_content([{"text": schema_prompt}])
